...
def get_candlestick_features(ohlcv_bars, resistance_1):
    ...
    for i, bar in enumerate(ohlcv_bars):
        ...
        tag = f"bar_{i+1}"
        features[f"{tag}_body_ratio"] = round(body / total_range, 2)
        features[f"{tag}_upper_wick_ratio"] = round(upper_wick / total_range, 2)
        features[f"{tag}_lower_wick_ratio"] = round(lower_wick / total_range, 2)
        features[f"{tag}_is_bearish"] = int(c < o)
        features[f"{tag}_is_bullish"] = int(c > o)
    ...
    features["is_shooting_star"] = int(upper_wick > 2 * abs(c - o) and (c - l) / total_range < 0.3 and c < o)
    features["is_hammer"] = int(lower_wick > 2 * abs(c - o) and (h - c) / total_range < 0.3 and c > o)

    # Engulfing patterns
    prev = ohlcv_bars[-2]
    features["is_bearish_engulfing"] = int(c < o and prev["close"] > prev["open"] and o >= prev["close"] and c <= prev["open"])
    features["is_bullish_engulfing"] = int(c > o and prev["close"] < prev["open"] and o <= prev["close"] and c >= prev["open"])

    # 5-bar trends
    closes = [b["close"] for b in ohlcv_bars]
    features["5bar_close_slope"] = round((closes[-1] - closes[0]) / (closes[0] + 1e-9), 4)

    volumes = [b["volume"] for b in ohlcv_bars]
    features["volume_trend_slope"] = round((volumes[-1] - volumes[0]) / (volumes[0] + 1e-9), 4)

    # Resistance Rejection
    touched_res = abs(h - resistance_1) / (resistance_1 + 1e-9) < 0.015
    strong_upper_wick = upper_wick / total_range > 0.40
    features["is_rejecting_resistance"] = int(touched_res and strong_upper_wick)
...
def fetch_stock_profile(ticker, target_date_str=None):
    try:
        ...
        # MULTI-TIMEFRAME: Calculate Weekly 20-EMA Trend
        try:
            weekly_closes = hist['Close'].resample('W').last()
            if len(weekly_closes) >= 20:
                weekly_ema20 = weekly_closes.ewm(span=20, adjust=False).mean().iloc[-1]
                weekly_trend = "Bullish" if current_close > weekly_ema20 else "Bearish"
            else:
        ...
        # ENGINEERED CANDLESTICK FEATURES
        candlestick_features = get_candlestick_features(last_5_days_ohlcv, resistance_1)
            
        technical_data = {
            "timeframe": "Daily (1D)",
            "current_price": current_close,
            "last_5_days_ohlcv": last_5_days_ohlcv,
            "candlestick_patterns": candlestick_features, 
            "graph_trend": trend,
            "ema_200": ema_200,
            "vwap_20d": vwap_val,
            "bollinger_bands": {
